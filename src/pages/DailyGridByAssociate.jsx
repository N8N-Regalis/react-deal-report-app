...
export default function DailyGridByAssociate() {
  ...
  const grid = useMemo(() => {
    ...
    return Object.entries(assocMap)
      .map(([associate, days]) => {
        const total = Object.values(days).reduce((s, v) => s + v, 0)
        return { associate, days, total }
      })
      .sort((a, b) => a.associate.localeCompare(b.associate))
  }, [unique])
        ...
        <div className="px-6">
          <div className="card overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {grid.map(row => (
                  <tr key={row.associate} className="hover:bg-gray-50">
